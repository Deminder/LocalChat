...
class MemberServiceImpl(
    ...
    override fun upsertMember(conversationId: Long, userId: Long, newPermission: Permission, color: Int?): Member =
        subjectAndAuthorPair(conversationId, userId).let { (subject, author) ->
            subject.copy(
                permission = authorizedPermissionModify(
                    subject.id == author.id,
                    subject.permission, newPermission, author.permission
                ),
                color = if (subject.id == author.id) color else subject.color
            )
                .let {
                    if (!subject.permission.administrate ||
                        newPermission.administrate ||
    ...
    private fun modificationPermission(
        selfSubject: Boolean,
    ...
    private fun authorizedPermissionModify(
        ss: Boolean,
        op: Permission,
        np: Permission,
        ap: Permission
    ) = modificationPermission(ss, op, ap).let { cp ->
        Permission(
