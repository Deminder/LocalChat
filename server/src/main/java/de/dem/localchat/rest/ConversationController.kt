...
class ConversationController(
    ...
    fun upsertMessage(
        ...
        @RequestBody r: MessageUpsertRequest
        ...
        return conversationService.upsertMessage(cid, r.messageId, r.text)
            .toConversationMessageDto().also {
                eventSubscriptionService.notifyMembers(
                    ConversationEvent(
                        "upsert-message",
                        mapOf("conversationId" to cid, "message" to it)
                    ), cid, username()
                )
            }
    }
    ...
    fun searchMessages(
        @PathVariable("cid") cid: Long,
        @RequestBody r: MessageSearchRequest
    ): ConversationMessagePageDto {
        return conversationService.conversationMessagePage(
            cid, r.page, r.pageSize,
            Instant.ofEpochMilli(r.olderThan), Instant.ofEpochMilli(r.newerThan),
            r.search, r.regex
        ).toConversationMessagePageDto(cid, r)
    }
    ...
    fun deleteMessage(@PathVariable("cid") cid: Long, @PathVariable("mid") messageId: Long) {
        conversationService.deleteMessage(cid, messageId).also {
            eventSubscriptionService.notifyMembers(
                ConversationEvent(
                    "delete-message",
                    mapOf("conversationId" to cid, "messageId" to messageId)
                ), cid, username()
            )
        }
    }
    ...
    fun upsertMember(
        ...
        updateRequest.permission?.let {
            toMemberDto(memberService.upsertMember(
                cid, uid,
                it.toPermission(),
                updateRequest.color
            ).also { member ->
    ...
    fun deleteMember(
        ...
        return memberService.removeMember(cid, uid).also {
            eventSubscriptionService.notifyMembers(
                ConversationEvent(
                    "delete-member",
                    mapOf("conversationId" to cid, "userId" to uid)
                ), cid, username()
            )
        }
    }
